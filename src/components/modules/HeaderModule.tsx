...
export const HeaderModule = ({ data, modules = [], isPreview, onCTA, onUpdate, config }: HeaderModuleProps) => {
  ...
  const projectLogo = config?.project?.logoUrl;
  const projectBusinessName = config?.project?.name || config?.profile?.businessName || config?.userProfile?.businessName;
  ...
  const projectSocials = config?.project?.socials || {};
