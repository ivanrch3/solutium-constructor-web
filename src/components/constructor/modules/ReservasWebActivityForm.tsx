...
} : { catalog_item_id: '', title_override: '', modality: 'presencial', physical_location: '', maps_url: '', private_virtual_url: '', timezone, total_capacity: 1, is_free: true, regular_price: 0, promotional_price: null, promotion_ends_at: null, currency: null, sinpe_phone: null, payment_receipt_whatsapp: null, onvopay_url: null, selected_whatsapp_channel_id: null, sessions: [blankSession()] };
...
export const validateReservasWebActivityDraft = (draft: Draft, eligibleWhatsAppChannels: ReservasWebEligibleWhatsAppChannel[] = []) => {
  ...
  if (!draft.is_free) {
    ...
    const validSinpe = Boolean(optional(draft.sinpe_phone)) && /^\+?[0-9\s()\-]{7,20}$/.test(String(draft.sinpe_phone));
...
const fields: Array<keyof Omit<Draft, 'sessions'>> = ['catalog_item_id','title_override','short_description_override','long_description_override','facilitator_name','facilitator_whatsapp','modality','physical_location','maps_url','private_virtual_url','timezone','total_capacity','is_free','regular_price','promotional_price','promotion_ends_at','currency','sinpe_phone','payment_receipt_whatsapp','onvopay_url','selected_whatsapp_channel_id'];
...
export const ReservasWebActivityForm = ({ mode, projectId, products, detail, timezone, eligibleWhatsAppChannels = [], onClose, onSaved }: Props) => {
  ...
  const setFree = (isFree: boolean) => isFree ? setDraft(current => ({ ...current, is_free: true, regular_price: 0, promotional_price: null, promotion_ends_at: null })) : update('is_free', false);
  ...
  return <div className="mt-3 space-y-3 rounded-xl border border-primary/30 bg-background p-3 text-xs"><div className="flex items-center justify-between"><h3 className="font-semibold">{mode === 'create' ? 'Crear actividad' : 'Editar actividad'}</h3><button type="button" onClick={onClose} className="text-primary underline">Cerrar</button></div>
    ...
    <section className="space-y-2 border-t border-border/40 pt-2"><h4 className="font-semibold">Pago</h4><div className="flex gap-3"><label><input type="radio" checked={draft.is_free} onChange={() => setFree(true)} /> Gratuita</label><label><input type="radio" checked={!draft.is_free} onChange={() => setFree(false)} /> De pago</label></div>{!draft.is_free && <><div className="grid grid-cols-2 gap-2"><label>Precio regular<input type="number" min="0.01" step="0.01" value={draft.regular_price ?? ''} onChange={event => update('regular_price', Number(event.target.value))} className="mt-1 w-full rounded border border-border bg-surface p-2" /></label><label>Moneda<input value={draft.currency || ''} maxLength={3} onChange={event => update('currency', event.target.value.toUpperCase())} className="mt-1 w-full rounded border border-border bg-surface p-2" /></label></div><div className="grid grid-cols-2 gap-2"><label>Número SINPE<input value={draft.sinpe_phone || ''} onChange={event => update('sinpe_phone', event.target.value)} className="mt-1 w-full rounded border border-border bg-surface p-2" /></label><label>Número para recibir comprobantes<input value={draft.payment_receipt_whatsapp || ''} onChange={event => update('payment_receipt_whatsapp', event.target.value)} className="mt-1 w-full rounded border border-border bg-surface p-2" /></label></div><label className="block">URL de pago con tarjeta (Onvopay)<input value={draft.onvopay_url || ''} onChange={event => update('onvopay_url', event.target.value)} className="mt-1 w-full rounded border border-border bg-surface p-2" /></label><div className="grid grid-cols-2 gap-2"><label>Precio promocional<input type="number" min="0.01" step="0.01" value={draft.promotional_price ?? ''} onChange={event => update('promotional_price', event.target.value ? Number(event.target.value) : null)} className="mt-1 w-full rounded border border-border bg-surface p-2" /></label><label>Fin de promoción<input type="datetime-local" value={draft.promotion_ends_at || ''} onChange={event => update('promotion_ends_at', event.target.value || null)} className="mt-1 w-full rounded border border-border bg-surface p-2" /></label></div>{(draft.promotional_price || draft.promotion_ends_at) && <button type="button" onClick={() => setDraft(current => ({ ...current, promotional_price: null, promotion_ends_at: null }))} className="text-primary underline">Quitar promoción</button>}</>}</section>
