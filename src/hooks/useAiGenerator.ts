...
export const useAiGenerator = (config: any) => {
  ...
  const handleGenerateAi = async (
    ...
    const projectBusinessName = config?.project?.name || (config as any)?.projectsData?.name || (config as any)?.projectData?.name || config?.profile?.businessName || (config as any)?.profilesData?.businessName || (config as any)?.userProfile?.businessName;
