...
export type PublicReservasWebReservationInput = { holdToken: string; idempotencyKey: string; contactFirstName: string; contactLastName: string; contactWhatsapp: string; paymentMethod: 'sinpe' | 'card' | null; paymentMode?: 'full' | 'deposit' | null; participants: Array<{ firstName: string; lastName: string; sex: 'male' | 'female' | 'prefer_not_to_say'; age: number; identificationNumber: string }> };
export type PublicReservasWebReservation = { reservationReference: string; status: 'confirmed' | 'pending_payment'; confirmedAt: string | null; reservedAt: string | null; paymentDueAt: string | null; quantity: number; totalAmount: number; requiredNow: number; pendingBalance: number; perPersonPaymentAmount: number | null; paymentMode: 'full' | 'deposit'; depositRefundable: boolean | null; currency: string; paymentUrl: string | null; paymentReceiptWhatsapp: string | null; payment: { method: 'free' } | { method: 'sinpe'; phone?: string; beneficiary?: string } | { method: 'card' } };
...
export const createPublicReservasWebReservation = async (input: PublicReservasWebReservationInput, signal?: AbortSignal, baseUrl = getAppMadreBaseUrl()): Promise<{ reservation: PublicReservasWebReservation; idempotentReplay: boolean }> => {
  ...
  const publicPayment: PublicReservasWebReservation['payment'] = method === 'sinpe' ? { method, ...(asText(payment.phone) ? { phone: asText(payment.phone)! } : {}), ...(asText(payment.beneficiary) ? { beneficiary: asText(payment.beneficiary)! } : {}) } : method === 'card' ? { method } : { method: 'free' };
  return { reservation: { reservationReference: reference, status, confirmedAt: asText(reservation.confirmedAt), reservedAt: asText(reservation.reservedAt), paymentDueAt: asText(reservation.paymentDueAt), quantity: asNumber(reservation.quantity ?? reservation.participantCount), totalAmount: asNumber(reservation.totalAmount), requiredNow: asNumber(reservation.requiredNow ?? reservation.requiredAmount ?? reservation.totalAmount), pendingBalance: asNumber(reservation.pendingBalance), perPersonPaymentAmount: asOptionalNumber(reservation.perPersonPaymentAmount) ?? null, paymentMode: reservation.paymentMode === 'deposit' ? 'deposit' : 'full', depositRefundable: typeof reservation.depositRefundable === 'boolean' ? reservation.depositRefundable : null, currency: asText(reservation.currency) || 'USD', paymentUrl: asText(reservation.paymentUrl) ?? asText(payment.paymentUrl), paymentReceiptWhatsapp: asText(reservation.paymentReceiptWhatsapp) ?? asText(payment.receiptWhatsapp), payment: publicPayment }, idempotentReplay: payload.idempotentReplay === true };
};
