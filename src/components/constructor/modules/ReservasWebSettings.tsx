  ...
  return (
    <div className="mt-3 space-y-3 rounded-xl border border-border/40 bg-surface p-3 text-sm text-text">
      <section className="space-y-2"><h3 className="text-xs font-semibold">Actividad</h3>
        ...
        {selectedActivity && !form && <div className="space-y-2 rounded-lg border border-border/40 p-3 text-xs"><p className="font-semibold">{selectedActivity.catalogItemName || 'Sin item de catálogo'}{isReservasWebActivityArchived(selectedActivity) ? ' · Archivada' : ''}</p><p>{selectedActivity.facilitator || 'Facilitador no definido'} · {selectedActivity.modality || 'Modalidad no definida'}</p><p>{formatReservasWebSessionSummary(selectedActivity)}</p><p>Capacidad total: {selectedActivity.totalCapacity ?? 'No definida'}</p><p>{formatReservasWebPrice(selectedActivity)}</p><p>{getReservasWebActivityReadinessMessage(selectedActivity)}</p><div className="flex gap-2"><button type="button" disabled={!projectId || loadingDetail} onClick={openEdit} className="rounded border border-primary/40 px-2 py-1 font-semibold text-primary disabled:opacity-40">{loadingDetail ? 'Cargando…' : 'Editar actividad'}</button><button type="button" onClick={() => persistActivityId(null)} className="font-semibold text-primary underline">Cambiar actividad</button></div></div>}
      </section>
