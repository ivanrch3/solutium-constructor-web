...
export const BentoCompositeContent = ({ item, darkMode, breakpoint = 'desktop', isPreviewMode }: any) => {
  ...
  const renderElement = (element: BentoCompositeElement) => {
    ...
    if (element.type === 'title') return <h3 key={element.id} style={{ ...commonTextStyle, textAlign }}>{element.text}</h3>;
    if (element.type === 'description') return <p key={element.id} style={{ ...commonTextStyle, color: element.color || mutedColor, textAlign }}>{element.text}</p>;
