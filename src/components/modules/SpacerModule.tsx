...
export const SpacerModule = ({ data, isPreview, onUpdate }: SpacerModuleProps) => {
  ...
  const adjustedHeight = isMobileSimulated ? Math.max(20, height * 0.6) : height;
  ...
  const renderSpacer = () => {
    switch (spacerType) {
      ...
      case 'line':
        return (
          <div className={`flex justify-center ${isMobileSimulated ? 'py-2' : 'py-4'}`}>
            <div 
              style={{ 
                width: `${isMobileSimulated ? Math.min(lineWidth, 100) : lineWidth}%`, 
      ...
      case 'icon':
        ...
        return (
          <div className={`flex items-center gap-4 ${isMobileSimulated ? 'py-2' : 'py-4'}`}>
            <div className="flex-1" style={{ borderTop: `${lineThickness}px ${lineStyle} ${lineColor}` }} />
            <IconComponent size={isMobileSimulated ? 16 : 20} style={{ color: lineColor }} />
            <div className="flex-1" style={{ borderTop: `${lineThickness}px ${lineStyle} ${lineColor}` }} />
          </div>
        );
        ...
        return (
          <div className={`flex items-center gap-4 ${isMobileSimulated ? 'py-2' : 'py-4'}`}>
            ...
            <Typography 
              variant="small" 
              weight="900" 
              className={`uppercase tracking-[0.2em] whitespace-nowrap ${isMobileSimulated ? 'text-[8px]' : ''}`}
      ...
      case 'pattern':
        return (
          <div 
            className={`w-full ${isMobileSimulated ? 'py-4' : 'py-8'}`} 
            style={{ 
      ...
      case 'scroll':
        return (
          <div className={`flex flex-col items-center gap-2 ${isMobileSimulated ? 'py-4' : 'py-8'}`}>
            <div className={`${isMobileSimulated ? 'h-8' : 'h-16'} w-px bg-gradient-to-b from-transparent to-current`} style={{ color: lineColor }} />
  ...
  return (
    ...
    >
      <div className={`max-w-7xl mx-auto ${isMobileSimulated ? 'px-4' : 'px-6'}`}>
        {renderSpacer()}
      </div>
    </ModuleWrapper>
  );
