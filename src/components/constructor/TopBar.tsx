  ...
  <div className={`bg-surface border-b border-border/60 grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 px-4 md:px-6 z-20 ${isMobile ? 'h-[70px]' : 'h-[60px]'}`}>
    ...
    <div className="flex items-center justify-end gap-2 md:gap-4">
      ...
      <div className="flex items-center gap-1.5 md:gap-2">
        ...
        <motion.button 
          whileHover={(publishStatus === 'idle' && !isSaving && !hasUnsavedChanges && currentStatus !== 'published') ? { scale: 1.02 } : {}}
          whileTap={(publishStatus === 'idle' && !isSaving && !hasUnsavedChanges && currentStatus !== 'published') ? { scale: 0.98 } : {}}
          onClick={(publishStatus === 'idle' && !isSaving && !hasUnsavedChanges && currentStatus !== 'published') ? onPublish : undefined}
          disabled={publishStatus !== 'idle' || isSaving || hasUnsavedChanges || currentStatus === 'published'}
