...
test('public deposit amount uses depositAmountPerPerson rather than regular price', () => { const markup = render(); assert.match(markup, /Pago requerido para reservar.*25[.\s,]*000.*por persona/); assert.doesNotMatch(markup, /Pago requerido para reservar.*100[.\s,]*000.*por persona/); });
