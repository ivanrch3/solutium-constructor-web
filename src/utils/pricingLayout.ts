...
export const resolvePricingGridClass = (desktopColumns: number): string => {
  if (desktopColumns >= 4) return 'grid-cols-1 @lg:grid-cols-2 @5xl:grid-cols-4';
  if (desktopColumns === 3) return 'grid-cols-1 @lg:grid-cols-2 @5xl:grid-cols-3';
  if (desktopColumns === 2) return 'grid-cols-1 @lg:grid-cols-2';
  return 'grid-cols-1';
};
