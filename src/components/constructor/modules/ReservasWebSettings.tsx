...
export const ReservasWebSettings = ({ moduleId, projectId = null, products = [], settingsValues, reservasWebActivities = [], reservasWebEligibleWhatsAppChannels = [], onActivitiesRefreshed, onSettingChange }: ReservasWebSettingsProps) => {
  ...
  const [form, setForm] = useState<{ mode: ReservasWebActivityFormMode; detail?: ReservasWebActivityAdminDetail; archiveConfirmationInitiallyOpen?: boolean } | null>(null);
  const [loadingDetail, setLoadingDetail] = useState(false);
  const openEdit = async (archiveConfirmationInitiallyOpen = false) => { if (!projectId || !selectedActivityId) return; setLoadingDetail(true); try { setForm({ mode: 'edit', detail: await getReservasWebActivity(projectId, selectedActivityId), archiveConfirmationInitiallyOpen }); } finally { setLoadingDetail(false); } };
  const saved = async (detail: ReservasWebActivityAdminDetail) => { const summary = asSummary(detail); const next = [...reservasWebActivities.filter(item => item.id !== detail.id), summary]; onActivitiesRefreshed?.(next); if (form?.mode === 'create') { persistActivityId(detail.id); setForm(null); } else setForm({ mode: 'edit', detail }); };
  ...
  return (
    <div className="mt-3 space-y-3 rounded-xl border border-border/40 bg-surface p-3 text-sm text-text">
      <section className="space-y-2"><h3 className="text-xs font-semibold">Actividad</h3>
        ...
        <div className="flex gap-2"><button type="button" disabled={!projectId} onClick={() => setForm({ mode: 'create' })} className="rounded border border-primary/40 px-2 py-1 font-semibold text-primary disabled:opacity-40">Crear actividad</button></div>
        {form && projectId && <ReservasWebActivityForm mode={form.mode} projectId={projectId} products={products} detail={form.detail} eligibleWhatsAppChannels={reservasWebEligibleWhatsAppChannels} display={config.display} reserveButtonLabel={config.content.reserveButtonLabel} onDisplayChange={updateDisplay} onReserveButtonLabelChange={(value) => persist({ ...config, content: { reserveButtonLabel: value } })} archiveConfirmationInitiallyOpen={form.archiveConfirmationInitiallyOpen} onClose={() => setForm(null)} onSaved={saved} />}
        ...
        {selectedActivity && !form && <div className="space-y-2 rounded-lg border border-border/40 p-3 text-xs"><p className="font-semibold">{selectedActivity.catalogItemName || 'Sin item de catálogo'}{isReservasWebActivityArchived(selectedActivity) ? ' · Archivada' : ''}</p><p>{selectedActivity.facilitator || 'Facilitador no definido'} · {selectedActivity.modality || 'Modalidad no definida'}</p><p>{formatReservasWebSessionSummary(selectedActivity)}</p><p>Capacidad total: {selectedActivity.totalCapacity ?? 'No definida'}</p><p>{formatReservasWebPrice(selectedActivity)}</p><p>{getReservasWebActivityReadinessMessage(selectedActivity)}</p><div className="flex flex-wrap gap-2"><button type="button" disabled={!projectId || loadingDetail} onClick={() => void openEdit()} className="rounded border border-primary/40 px-2 py-1 font-semibold text-primary disabled:opacity-40">{loadingDetail ? 'Cargando…' : 'Editar actividad'}</button><button type="button" disabled={!projectId || loadingDetail || isReservasWebActivityArchived(selectedActivity)} onClick={() => void openEdit(true)} className="rounded border border-border px-2 py-1 font-semibold disabled:opacity-40">Archivar</button></div></div>}
      </section>
