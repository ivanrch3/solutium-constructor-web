...
export const PropertyInspector = ({ 
  ...
  const projectSocials = config?.project?.socials || (config as any)?.projectsData?.socials || (config as any)?.projectData?.socials;

  const isPremiumUser = config?.profile?.subscriptionPlan === 'pro' || config?.profile?.subscriptionPlan === 'premium' || (config as any)?.profilesData?.subscriptionPlan === 'pro';
    ...
    .map(f => {
      ...
      if (selectedModule) {
        ...
        if (selectedModule.type === 'footer' && f.name === 'logoText') {
          const projectLogo = config?.project?.logoUrl || (config as any)?.projectsData?.logoUrl || (config as any)?.projectData?.logoUrl;
