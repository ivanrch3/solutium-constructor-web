...
export type SpecialEventPhoto = { id: string; imageUrl: string; contributor: { id: string; name: string; message?: string | null }; createdAt: string; status?: 'pending' | 'approved' | 'hidden'; updatedAt?: string };
...
export const specialEventApi = {
  ...
  async getPhotos(slug: string, signal?: AbortSignal): Promise<SpecialEventPhoto[]> { const r = await fetch(path(slug, '/photos'), { signal }); if (!r.ok) await error(r); return (await r.json()).photos || []; },
